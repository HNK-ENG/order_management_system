...
import {Axios, SALES_ORDER_HISTORY, SALES_ORDER_DISPATCHED} from '../../api/axios';
import {ADMIN_TABLE_HEADS, ADMIN_ACTION_ENABLED_STATUS, ToTitleCase, GetAdminActionByStatus} from '../../context/Globals';
...
const Home = () => {
    ...
    const getSalesOrderHistory = async (status) => {
        try{
            ...
            { params: { pageNumber: 1, itemsPerPage:1000, status } });
    ...
    const onActionClicked = async (obj) => {
        try{
            ...
            switch(obj.status){
                case "NEW":
                    await Axios.get(SALES_ORDER_DISPATCHED + "/" + obj.orderId);
                    break;
            }
            getSalesOrderHistory(status);
        }catch (err) {
    ...
    return (
        <div className="home">
            ...
            <Table striped bordered hover size="sm">
                <thead>
                    <tr>
                        {ADMIN_TABLE_HEADS.map((head, index) => <th key={index}>{head}</th>)}
                    </tr>
                </thead>
                <tbody>
                        {data?.map((obj, _) => (
                            <tr>
                                ...
                                <td>{obj.quantity}</td>
                                <td>{obj.totalCost}</td>
                                <td>{obj.status.toUpperCase() in ADMIN_ACTION_ENABLED_STATUS && GetAdminActionByStatus(obj.status.toUpperCase()) ?
                                    <Button
                                        size="sm"
                                        variant={obj.status.toUpperCase()=== "NEW"? "outline-info": null}
                                        onClick={()=>onActionClicked(obj)}>{GetAdminActionByStatus(obj.status.toUpperCase())}
                                    </Button> : null}
                                </td>
                            </tr>
                        ))}
